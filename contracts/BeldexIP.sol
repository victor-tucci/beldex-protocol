...
contract BeldexIP {
    ...
    function verifyInnerProduct(Utils.G1Point[] memory _hs, Utils.G1Point memory u, Utils.G1Point memory P, Proof memory proof, uint256 salt) public view returns (bool) {
    ...
    function verify(Statement memory statement, Proof memory proof, uint256 salt) internal view returns (bool) {
        ...
        ip_info.o = salt; 
        ip_info.challenges = new uint256[](log_n);
        for (uint256 i = 0; i < log_n; i++) {
            ip_info.o = uint256(keccak256(abi.encode(ip_info.o, proof.ls[i], proof.rs[i]))).gMod(); 
            ip_info.challenges[i] = ip_info.o;
            statement.P = statement.P.pAdd(proof.ls[i].pMul(ip_info.o.gExp(2)).pAdd(proof.rs[i].pMul(ip_info.o.gInv().gExp(2))));
        }

        ip_info.otherExponents = new uint256[](n);
        ip_info.otherExponents[0] = 1;
        for (uint256 i = 0; i < log_n; i++) {
            ip_info.otherExponents[0] = ip_info.otherExponents[0].gMul(ip_info.challenges[i]);
        }
        bool[] memory bitSet = new bool[](n);

        ip_info.otherExponents[0] = ip_info.otherExponents[0].gInv();
        for (uint256 i = 0; i < n / 2; ++i) {
            for (uint256 j = 0; (1 << j) + i < n; ++j) {
                uint256 i1 = i + (1 << j);
                if (!bitSet[i1]) {
                    uint256 temp = ip_info.challenges[log_n - 1 - j].gExp(2);
                    ip_info.otherExponents[i1] = ip_info.otherExponents[i].gMul(temp);
                    bitSet[i1] = true;
                }
            }
        }
        ...
        for (uint256 i = 0; i < n; i++) {
            gTemp = gTemp.pAdd(gs(i).pMul(ip_info.otherExponents[i]));
            hTemp = hTemp.pAdd(statement.hs[i].pMul(ip_info.otherExponents[n - 1 - i]));
        }
